    ...
    public BigDecimal readBigDecimal(String prompt, BigDecimal min, BigDecimal max) {
        ...
        do {
            ...
            if(result.compareTo(min) == -1){
                this.print("entered area is below minimum");
            }
            if(result.compareTo(max) == 1){
                this.print("entered area is Above maximum");
            }
    ...
    public LocalDate readLocalDate(String prompt, LocalDate min, LocalDate max) {
        ...
        do {
            ...
            if(result.compareTo(min) <= 0){
                this.print("order date must be future date.");
            }
            if(result.compareTo(max) >= 1){
                this.print("enter closer date. entered date is too far from current date.");
            }
        } while (result.compareTo(min) <= 0 || result.compareTo(max) >= 1);
        return result;
    }
}
